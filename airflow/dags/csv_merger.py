...
def etl_process(**kwargs):
    ...
    for csv_file in glob(file_path+"data/time_series_covid19_*_global.csv"):

        match = re.search('time_series_covid19_(confirmed|deaths|recovered)_global.csv', os.path.basename(csv_file))
        if match:
            logger.info("Processing csv file " + csv_file)
            ...
            #transform dates columns to rows
            ...
            with mysql_connection.begin() as connection:
                connection.execute("DELETE FROM covid19.merged WHERE status='"+covid_status+"'")
                transformed_df.to_sql('global_data', con=connection, schema='padp_db', if_exists='append', index=False)



            logger.info(f"Rows inserted {len(df.index)}")
        else:
            logger.warning("Not valid covid filename status: " + csv_file)

    #Clean all files
    logger.info("Cleaning files...")
    for file_name in  glob(file_path + "data/*"):
        logger.info("Deleting filename: "+file_name)
        os.remove(file_name)


dag = DAG('csv_merger', description='A simple merge and transform of CSVs on data folder',
          default_args={
              'owner': 'Carlos Avendano',
...
etl = PythonOperator(task_id="csv_merger_etl",
