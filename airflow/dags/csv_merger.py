...
def etl_process(**kwargs):
    ...
    for covid_file in glob(file_path+"data/time_series_covid19_*_global.csv"):
        ...
        if match:
            ...
            with mysql_connection.begin() as connection:
                connection.execute("DELETE FROM covid19.data WHERE status='"+covid_status+"'")
                transformed_df.to_sql('data', con=connection, schema='covid19', if_exists='append', index=False)
